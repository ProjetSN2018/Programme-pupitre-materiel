...
#define _LcdWriteInst(inst)			Lcd(_LCD_WRITE_INST,(uint32_t)inst)							//Permet d'écrire des instructions pour le LCD (ex : display ON/OFF)
#define _LcdWriteData(data)			Lcd(_LCD_WRITE_DATA,(uint32_t)data)							
#define _LcdFindCenter(len)			Lcd(_LCD_FIND_CENTER,(uint32_t)len)							//Fonction permettant de trouver sur quelle colonne commencer l'écriture pour centrer l'affichage
#define _LcdEstrobe()				Lcd(_LCD_E_STROBE)											
#define _LcdEup()					Lcd(_LCD_E_UP)												//Met la pin E à 1	(Enable) Le lcd traite les données									
#define _LcdEdown()					Lcd(_LCD_E_DOWN)											//Met la pin E à 0	Le lcd ne traîte pas R/W et RS
#define _LcdSetCursor(line,col)		Lcd(_LCD_SET_RAM_ADDRESS,(uint32_t)line,(uint32_t)col)		//Fonction permettant de set la position du curseur	
...
const char line2addr[]={ 0x00, 0x40, 0x14, 0x54 };	//Adresse des lignes, respectivement, 0, 1, 2 et 3
char buf[16];
